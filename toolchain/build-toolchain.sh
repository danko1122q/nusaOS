...
make_toolchain_file () {
  ...
  $SED_BIN -i "s/@NUSAOS_SOURCE_DIR@/$(echo "$SOURCE_DIR" | sed -r 's/\//\\\//g')/g" "$BUILD/CMakeToolchain.txt"
  $SED_BIN -i "s/@NUSAOS_TARGET@/$TARGET/g" "$BUILD/CMakeToolchain.txt"
  $SED_BIN -i "s/@NUSAOS_ARCH@/$ARCH/g" "$BUILD/CMakeToolchain.txt"
}
