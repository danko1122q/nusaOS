...
export NUSAOS_PORTS_SCRIPT="${PORTS_DIR}/ports.sh"

source "$SOURCE_DIR/scripts/nusaos.sh"

# Setup environment
export CXX="i686-pc-nusaos-g++"
export CC="i686-pc-nusaos-gcc"
export RANLIB="i686-pc-nusaos-ranlib"
export AR="i686-pc-nusaos-ar"
export CXXFILT="i686-pc-nusaos-c++filt"
export READELF="i686-pc-nusaos-readelf"
export STRIP="$i686-pc-nusaos-strip"
export OBJCOPY="i686-pc-nusaos-objcopy"
...
apply_config_subs() {
  for CONFIGSUB in "${CONFIG_SUB_PATHS[@]}"; do
    ...
    "$SED_BIN" -i '/Each alternative MUST end in/a nusaos\* | \\' "$CONFIGSUB" || fail "Failed to automatically patch $CONFIGSUB"
    "$SED_BIN" -i '/sysv\* is not here because/a -nusaos\* | \\' "$CONFIGSUB" || fail "Failed to automatically patch $CONFIGSUB"
  done
}
...
install_dependencies() {
  msg "Installing dependencies for $NUSAOS_PORT_NAME..."
  ...
  success "Installed dependencies for $NUSAOS_PORT_NAME!"
}

build_port() {
  export NUSAOS_PORT_NAME="$1"
  export PORT_DIR="$PORTS_DIR/$NUSAOS_PORT_NAME"
  msg "Building port $NUSAOS_PORT_NAME..."
  
  # Default args
  export DOWNLOAD_URL=""
  export DOWNLOAD_FILE="$NUSAOS_PORT_NAME"
  export PATCH_FILE=""
  ...
  mkdir -p "$PORTS_DIR/build/$NUSAOS_PORT_NAME"
  pushd "$PORTS_DIR/build/$NUSAOS_PORT_NAME" || return 1
  ...
  if [[ $(type -t prebuild) == function ]]; then
    msg "Executing prebuild steps for $NUSAOS_PORT_NAME..."
    prebuild
  fi
  if [ "$USE_CONFIGURE" = "true" ]; then
    msg "Configuring port $NUSAOS_PORT_NAME..."
    if [ -z "$CONFIGURE_PATH" ]; then
      CONFIGURE_PATH="$DOWNLOAD_FILE/configure"
    fi
    "$CONFIGURE_PATH" --host="i686-pc-nusaos" "${CONFIGURE_ARGS[@]}" || return 1
  fi
  msg "Building port $NUSAOS_PORT_NAME..."
  make "-j$NUM_JOBS" "${MAKE_ARGS[@]}" || return 1
  msg "Installing port $NUSAOS_PORT_NAME..."
  make DESTDIR="$ROOT_DIR" "${INSTALL_ARGS[@]}" || return 1
  popd || return 1
  mkdir -p "$PORTS_D_DIR"
  touch "$PORTS_D_DIR/$NUSAOS_PORT_NAME"
  success "Built port $NUSAOS_PORT_NAME!"
}
