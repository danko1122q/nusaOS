...
rsync -aH --update -t -r "${SOURCE_DIR}/toolchain/tools/$ARCH/$ARCH-pc-nusaos/lib/"* "$FS_DIR"/lib || (echo "Couldn't copy libs." && exit 1)
rsync -aH --update -t -r "${SOURCE_DIR}/toolchain/tools/$ARCH/$ARCH-pc-nusaos/include/"* "$FS_DIR"/usr/include || (echo "Couldn't copy headers." && exit 1)
