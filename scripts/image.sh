...
if [ ! "$USE_EXISTING" ]; then
  ...
  if [ "$SYSTEM" = "Darwin" ]; then
    diskutil partitionDisk $(basename "$dev") 1 MBR fuse-ext2 nusaOS 100% || fail "Couldn't partition image."
  else
...
if [ ! "$USE_EXISTING" ]; then
  if [ "$SYSTEM" = "Darwin" ]; then
    ...
    # FIX: Tambahkan --locales="" untuk menghindari error penyalinan bahasa
    # dan --no-floppy untuk mempercepat/menghindari error probe.
    "$GRUB_COMMAND" --boot-directory=mnt/boot \
                    --target=i386-pc \
                    --locales="" \
                    --no-floppy \
                    --modules="ext2 part_msdos" \
                    "${dev}" --force || fail "Couldn't install grub."
