...
namespace IO {
	...
	public:
...
#endif
				return *((volatile T*) (m_vm_region->start() + offset));
		}
		...
		// BUG FIX: parameter sebelumnya T& (reference) sehingga rvalue/literal
		// tidak bisa dipakai langsung. Ganti ke T (by value) agar
		// window.out8(offset, 0x42) bisa dikompilasi.
		template<typename T>
...
#endif
				*((volatile T*) (m_vm_region->start() + offset)) = data;
		}

		void out8(size_t offset, uint8_t val) { out<uint8_t>(offset, val); }
		void out16(size_t offset, uint16_t val) { out<uint16_t>(offset, val); }
		void out32(size_t offset, uint32_t val) { out<uint32_t>(offset, val); }
