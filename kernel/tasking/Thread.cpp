        ...
        m_page_directory(process->_page_directory)
{
        ...
        // BUG FIX: konsistenkan dengan konstruktor lain, gunakan ->end()
        Stack stack((void*) _kernel_stack_region->end());
...
void Thread::block(Blocker& blocker) {
        if(state() != ALIVE) {
                // Jika thread user sedang dying/zombie/dead, jangan PANIC —
                // cukup log dan return; thread ini akan segera direap.
                if (!is_kernel_mode()) {
                        KLog::warn("Thread",
                                "block() called on user thread {}(pid:{} tid:{}) in state '{}' — skipping block",
                                _process->name().c_str(), _process->pid(), _tid, state_name());
                        return;
                }
                // Thread kernel yang coba block saat tidak ALIVE → masih bug kernel
                PANIC("INVALID_BLOCK",
                        "Tried to block kernel thread %d of PID %d in state %s",
                        _tid, _process->pid(), state_name());
        }
        ASSERT(!_blocker);
        ...
        if(blocker.responsible_thread()) {
                ...
                if(responsible->is_blocked() && !responsible->should_unblock() && responsible->_blocker->responsible_thread() == this) {
                        // Jika keduanya thread user → bunuh kedua proses, jangan BSOD
                        if (!is_kernel_mode() && !responsible->is_kernel_mode()) {
                                KLog::warn("Thread",
                                        "User-space deadlock detected between {}[{}.{}] and {}[{}.{}] — killing both",
                                        _process->name().c_str(), _process->pid(), _tid,
                                        responsible->_process->name().c_str(), responsible->_process->pid(), responsible->_tid);
                                responsible->_process->kill(SIGKILL);
                                _process->kill(SIGKILL);
                                return;
                        }
                        // Deadlock melibatkan thread kernel → masih bug sejati
                        PANIC(
                                "THREAD_DEADLOCK",
...
void Thread::reap() {
        ...
        if(m_next)
                m_next->m_prev = m_prev; // BUG FIX: sebelumnya = this, menyebabkan dangling pointer setelah thread di-free
}
