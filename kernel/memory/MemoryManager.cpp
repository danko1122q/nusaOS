...
void MemoryManager::page_fault_handler(ISRRegisters* regs) {
        ...
        switch (regs->err_code) {
                // ── Kernel-space faults → selalu bug kernel → PANIC ──────────────
                ...
                // ── User-space faults → bunuh proses saja, JANGAN BSOD ───────────
                case FAULT_USER_READ:
                case FAULT_USER_READ_GPF:
                case FAULT_USER_WRITE:
                case FAULT_USER_WRITE_GPF: {
                        KLog::warn("Memory",
                                "User page fault (err_code={}) at {#x} — killing process",
                                regs->err_code, err_pos);
                        // Hanya kill jika ada proses user yang bisa dibunuh.
                        // Jika tidak ada (early boot / kernel thread), fallthrough ke PANIC.
                        if (TaskManager::enabled() && TaskManager::current_thread() &&
                                !TaskManager::current_thread()->is_kernel_mode()) {
                                critical.exit();
                                TaskManager::current_process()->kill(SIGSEGV);
                                return;
                        }
                        // Tidak ada proses user yang bisa dibunuh — ini bug kernel sejati.
                        PANIC("USR_FAULT_IN_KERNEL_CTX",
                                "User-space page fault in kernel context at 0x%x (err=%d)",
                                err_pos, regs->err_code);
                }

                default:
                        PANIC("UNKNOWN_PAGE_FAULT", "err_code=%d addr=0x%x", regs->err_code, err_pos);
        }
#endif
        // TODO: aarch64
}
