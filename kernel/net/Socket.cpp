...
ResultRet<kstd::Arc<Socket>> Socket::accept(FileDescriptor& fd, UserspacePointer<sockaddr> addr, UserspacePointer<socklen_t> addrlen, int options) {
	...
	auto accept_res = client->do_accept();
	if (accept_res.is_error()) {
		// BUG FIX: sebelumnya jika do_accept() gagal, client hilang tanpa dikembalikan ke backlog.
		// Kembalikan client ke backlog (push_back karena kstd::queue tidak punya push_front).
		LOCK(m_lock);
		m_client_backlog.push_back(kstd::move(client));
		return accept_res;
	}
