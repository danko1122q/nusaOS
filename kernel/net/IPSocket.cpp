...
ResultRet<kstd::Arc<IPSocket>> IPSocket::make(Socket::Type type, int protocol) {
	...
	case Type::Raw:
		// Protocol 1 = ICMP (IPPROTO_ICMP tidak didefinisikan di kernel headers, pakai nilai numerik)
		if (protocol == 1)
			return kstd::static_pointer_cast<IPSocket>(TRY(ICMPSocket::make()));
		return Result(EPROTONOSUPPORT);
	default:
