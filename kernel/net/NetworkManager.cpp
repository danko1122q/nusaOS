...
void NetworkManager::handle_arp(const kstd::Arc<NetworkAdapter>& adapter, const NetworkAdapter::Packet* raw_packet) {
	if (raw_packet->size < (sizeof(NetworkAdapter::FrameHeader) + sizeof(ARPPacket))) {
		KLog::warn("NetworkManager", "Got ARP packet with invalid frame size!");
		return; // BUG FIX: sebelumnya tidak return, lanjut proses packet invalid
	}
...
void NetworkManager::handle_icmp(const kstd::Arc<NetworkAdapter>& adapter, const IPv4Packet& packet) {
	...
	if ((ICMPType) header.type == ICMPType::EchoRequest) {
		// Balas Echo Request dengan Echo Reply
		if (packet.length < (sizeof(IPv4Packet) + sizeof(ICMPEchoPacket))) {
			KLog::warn("NetworkManager", "Received ICMP Echo Request of invalid size!");
			return;
		}

		// Gunakan MAC dari ARP cache saja - tidak boleh block di NetworkManager thread
		// ARP cache sudah diisi saat menerima packet dari sender di handle_ipv4
		auto mac_res = Router::arp_lookup(packet.source_addr);
		if (mac_res.is_error())
			return;
		Router::Route route { mac_res.value(), adapter };

		// Payload size = total ICMP data (echo packet + any extra payload)
		size_t icmp_size = packet.length.val() - sizeof(IPv4Packet);
		auto pkt_res = route.adapter->alloc_packet(sizeof(IPv4Packet) + icmp_size);
		if (pkt_res.is_error())
			return;
		auto* pkt = pkt_res.value();

		auto* ipv4 = route.adapter->setup_ipv4_packet(pkt, route.mac, packet.source_addr, ICMP, icmp_size, 0, 64);
		auto* reply = (ICMPEchoPacket*) ipv4->payload;

		// Copy seluruh ICMP payload dari request (id, sequence, data)
		memcpy(reply, packet.payload, icmp_size);
		reply->header.type = (uint8_t) ICMPType::EchoReply;
		reply->header.code = 0;
		reply->header.checksum = 0;

	// Hitung checksum byte-by-byte untuk menghindari unaligned access pada packed struct
	uint32_t sum = 0;
	auto* cbytes = (const uint8_t*) reply;
	for (size_t i = 0; i + 1 < icmp_size; i += 2)
		sum += (uint16_t)(cbytes[i] << 8 | cbytes[i + 1]);
	if (icmp_size & 1)
		sum += (uint16_t)(cbytes[icmp_size - 1] << 8);
	while (sum >> 16)
		sum = (sum & 0xFFFF) + (sum >> 16);
	reply->header.checksum = ~(uint16_t) sum;

		route.adapter->send_packet(pkt);
		route.adapter->release_packet(pkt);

	} else if ((ICMPType) header.type == ICMPType::EchoReply) {
		// Kirim ke ICMPSocket yang menunggu
		ICMPSocket::deliver_packet(packet);
	}
}
...
void NetworkManager::handle_tcp(const kstd::Arc<NetworkAdapter>& adapter, const IPv4Packet& packet) {
	...
	if (!sock) {
		...
		// Send an RST to the sender - gunakan ARP cache saja, tidak boleh block
		auto mac_res = Router::arp_lookup(packet.source_addr);
		if (mac_res.is_error())
			return;
		Router::Route route { mac_res.value(), adapter };
