...
time_t RTC::timestamp() {
	...
	// Simpan tahun absolut sebelum subtract EPOCH untuk leap year check
	int absolute_year = years;
	years -= EPOCH;
	...
	// ✅ FIX BUG #2 & #3: Add leap day hanya jika tahun kabisat DAN sudah melewati February
	if(LEAPYEAR(absolute_year) && month > 2)
		secs += SECSPERDAY;

	// ✅ FIX BUG #1: Day dimulai dari 1, jadi kurangi 1 (day 18 = 17 hari penuh lewat)
