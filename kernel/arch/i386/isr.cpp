                ...
                if(regs->isr_num < 32) {
                        switch(regs->isr_num) {
                                ...
                                case 14: // Page fault
                                {
                                        ...
                                        // Tentukan apakah ini benar-benar fault di sisi kernel:
                                        //   • reserved-bit violation → selalu bug kernel
                                        //   • early boot (belum ada thread)
                                        //   • preempting DAN fault address di kernel space → bug kernel
                                        //     (preempting + user address: proses user crash saat context switch,
                                        //      masih bisa di-handle → biarkan turun ke handler biasa)
                                        const bool is_kernel_addr = (err_pos >= HIGHER_HALF);
                                        const bool force_panic_path =
                                                fault.type == PageFault::Type::Unknown ||
                                                !TaskManager::current_thread() ||
                                                (TaskManager::is_preempting() && is_kernel_addr);
