...
GameWidget::GameWidget()
{
    ...
    auto nusa_file = Gfx::Image::load(base_path / "nusa.png");
    if (nusa_file.is_error())
    {
        Duck::Log::errf("nusa file error: {}", nusa_file.result());
    }
    nusa = nusa_file.value();
    reset();
}
        ...
        for (int j = 0; j < board.rows(); j++)
        {
            ...
            else
            {
                ...
                else if (board.is_mine(i, j))
                {
                    ...
                    ctx.draw_image(nusa, {i * (CELL_SIZE + 1) + (CELL_SIZE - IMAGE_SIZE) / 2, j * (CELL_SIZE + 1) + (CELL_SIZE - IMAGE_SIZE) / 2});
                }
                else
                {
