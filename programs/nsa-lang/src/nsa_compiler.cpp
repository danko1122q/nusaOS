...
struct Symbol { uint8_t id; SymType type; bool is_internal; };

/* ── Function descriptor ─────────────────────────────────────────────
 *  Pass-1: scan all func..endfunc, record name/params/ret.
 *  Pass-2: compile body, fill in addr. Calls get patched at end.    */
...
struct FuncDef {
    ...
    std::string          ret_name;
    SymType              ret_type;
    size_t               addr;       /* bytecode address, filled in pass-2 */
...
struct CS {
    ...
    /* Local scope (active inside func..endfunc) */
    ...
    /* forward CALL patches: {patch_offset, func_name} */
    ...
    /* Lookup: locals first (if in func), then globals */
    ...
    bool intern(const std::string& name, SymType type, uint8_t& id_out) {
        if (in_func) {
            ...
            if (it!=locals.end()) {
                ...
                id_out=it->second.id; return true;
            }
    ...
    static const char* sym_type_name(SymType t) {
        switch(t){ case SYM_INT: return "int"; case SYM_STR: return "string"; case SYM_BOOL: return "bool"; }
        return "?";
    }
};

using Tok=NsaLexer::Token; using Toks=std::vector<Tok>; using TK=NsaLexer::TokenKind;
static bool is_ident(const Tok& t,const char* s){ return t.kind==TK::TK_IDENT&&t.text==s; }
...
static void parse_print(CS& cs,const Toks& t,bool newline){
    ...
    const Tok& arg=t[1];
    NsaOpcode sop=newline?OP_PRINT_STR:OP_PRINT_STR_NL;
    NsaOpcode vop=newline?OP_PRINT_VAR:OP_PRINT_VAR_NL;
...
static void parse_unary(CS& cs,const Toks& t,NsaOpcode op){
    ...
    if(op==OP_NOT&&type!=SYM_INT&&type!=SYM_BOOL){cs.error("'not' requires int or bool");return;}
    cs.emit(op);cs.emit(id);
}
...
/* ── Condition jump helper (unchanged from v2.0) ─────────────────── */
static bool emit_condition_jump(CS& cs,const Toks& t,size_t cs_,size_t ce_,bool jit,Block& blk){
    if(ce_-cs_==1){
...
static void parse_if(CS& cs,const Toks& t){
    ...
    Block blk; blk.kind=BLK_IF; blk.loop_start=0; blk.counter_id=0; blk.owns_counter=false;
...
static void parse_loop(CS& cs,const Toks& t){
    ...
    Block blk; blk.fwd_patch_pos=0;blk.loop_start=0;blk.counter_id=0;blk.owns_counter=false;
...
/* ── Function definition ─────────────────────────────────────────────
 * Syntax:
 *   func greet name age          // params default to int
 *   func add a b -> result       // -> declares return var (int)
 *   endfunc
 *
 * Call:
 *   call greet username years
 *   call add x y -> sum          // -> captures return value
 *
 * All variables inside a function are LOCAL — they have their own
 * slots (0..63) and never touch the global variable table.
 * ------------------------------------------------------------------ */
static void parse_func(CS& cs,const Toks& t){
    ...
    /* Parse params and optional -> retvar */
    bool arrow=false;
    for(size_t i=2;i<t.size();i++){
        /* detect -> (tokenized as single TK_OP "->" by lexer) */
        ...
        if(t[i].kind!=TK::TK_IDENT||NsaLexer::is_keyword(t[i].text)){
            cs.error("expected parameter name, got '"+t[i].text+"'");return;
        }
        ParamDef p; p.name=t[i].text; p.type=SYM_INT;
        fd.params.push_back(p);
    }

    if(fd.params.size()>NSA_MAX_LOCALS-1){
        cs.error("too many parameters (max 63)");return;
    }

    /* Emit JMP_FWD over the function body */
    cs.emit(OP_JMP_FWD);
    ...
    /* Register parameters as local vars at fixed slots 0..n-1 */
    ...
    /* If function has a return variable, pre-register it as local
       at slot = param_count so STORE_RET knows its slot index */
    if(fd.has_ret){
    ...
    Block blk;
    blk.kind=BLK_FUNC; blk.fwd_patch_pos=patch_pos;
    blk.loop_start=fd.addr; blk.counter_id=0; blk.owns_counter=false;
    cs.blk_stack.push_back(blk);
}

/* return [optional — just emits RET, return value lives in its local slot] */
...
static void parse_endfunc(CS& cs,const Toks&){
    ...
    cs.emit(OP_RET); /* implicit return */

    /* Patch JMP_FWD over the function body */
    size_t span=cs.here()-blk.fwd_patch_pos-2;
    if(span>0xFFFF){cs.error("function body too large");return;}
    cs.patch_u16(blk.fwd_patch_pos,(uint16_t)span);

    /* Also update the addr in funcs map (may differ if funcs registered earlier) */
...
/* call <fname> [arg1 arg2 ...] [-> retvar]
 *
 * Emits:
 *   LOAD_ARG 0 global_arg0
 *   LOAD_ARG 1 global_arg1
 *   ...
 *   CALL abs_addr
 *   STORE_RET global_retvar local_ret_slot  (only if -> retvar)
 */
static void parse_call(CS& cs,const Toks& t){
    ...
    /* Collect args and optional -> retvar */
    ...
    /* Emit LOAD_ARG for each argument (stages global → local slot before CALL) */
    for(size_t i=0;i<args.size();i++){
        uint8_t gid; SymType gt;
        if(!cs.lookup(args[i],gid,&gt))return;
        cs.emit(OP_LOAD_ARG);
        cs.emit((uint8_t)i);  /* local slot = param index */
        cs.emit(gid);          /* source global            */
    }
    ...
    /* Emit STORE_RET if caller wants the return value */
    if(has_ret_dst){
        if(!fd.has_ret){cs.error("function '"+fname+"' has no return value (no -> in func)");return;}
        /* Return local slot = param_count (it's always the slot after params) */
        uint8_t local_ret=(uint8_t)fd.params.size();
        /* Intern or find global destination */
        uint8_t gdst;
        if(!cs.intern(ret_dst,fd.ret_type,gdst))return;
        cs.emit(OP_STORE_RET);
        cs.emit(gdst);      /* global destination */
        cs.emit(local_ret); /* local source slot  */
    }
}
...
static void parse_end(CS& cs,const Toks&){
    ...
    case BLK_IF: case BLK_ELSE:{
        size_t span=cs.here()-blk.fwd_patch_pos-2;
        if(span>0xFFFF){cs.error("block body too large");return;}
...
CompileResult compile(const std::string& source,const char* filename){
    ...
    for(size_t i=0;i<cs.blk_stack.size();i++){
        ...
        fprintf(stderr,"%s: error: unclosed '%s' block (missing 'end'/'endfunc')\n",filename,s);
        cs.errors++;
    }
    /* Patch forward CALL references */
    for(auto& p:cs.call_patches){
        ...
        if(it==cs.funcs.end()||it->second.addr==0){
            ...
            cs.patch_u16(p.first,(uint16_t)it->second.addr);
        }
    }
