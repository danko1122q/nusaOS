...
/*
 * NSA Compiler  v2.4  —  added .nss module system
 *
 * .nss files work like a header + implementation combined:
 *   - Declare global variables with  `global int/str/bool <name> = <val>`
 *   - Declare functions normally with `func` / `endfunc`
 *   - The entire .nss is available for import; only referenced symbols
 *     are included in the final .nbin (tree-shaking / dead-code elimination)
 *
 * In a .nsa file:
 *   import "math"           // looks for math.nss next to the .nsa file
 *   import "utils/strings"  // sub-path is fine
 *
 *   call math.max a b -> result
 *   let x = math.PI         // module globals become read-only aliases
 */
