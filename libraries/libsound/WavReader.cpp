...
#include "WavReader.h"
#include <libnusa/MappedBuffer.h>
...
WavReader::WavReader(Duck::File& file, Duck::Ptr<Duck::MappedBuffer> mapped_file, size_t data_chunk_start):
	m_file(file), m_mapped_file(mapped_file), m_header(*mapped_file->data<WavHeader>()), m_offset(data_chunk_start), m_data_chunk_start(data_chunk_start) {}
...
float WavReader::cur_time() const {
	// BUG FIX: gunakan m_data_chunk_start, bukan sizeof(WavHeader)
	// Data bisa dimulai jauh setelah WavHeader karena ada chunk-chunk lain
	return (float) ((m_offset - m_data_chunk_start) / bytes_per_sample()) / (float) m_header.sample_rate;
}

float WavReader::total_time() const {
	// BUG FIX: sama — gunakan m_data_chunk_start sebagai base
	return (float) ((m_mapped_file->size() - m_data_chunk_start) / bytes_per_sample()) / (float) m_header.sample_rate;
}

void WavReader::seek(float time) {
	LOCK(m_lock);
	// BUG FIX: gunakan m_data_chunk_start bukan sizeof(WavHeader) — seeknya jadi ke header, bukan data
	m_offset = m_data_chunk_start + ((size_t) (std::max(0.0f, std::min(time, total_time())) * m_header.sample_rate) * bytes_per_sample());
}
