...
void Window::set_show_min_max_buttons(bool show) {
	_show_min_max_buttons = show;
	if (!show) {
		_maximize_button.area = {0, 0, 0, 0};
	}
	repaint();
}
...
void Window::restore() {
	...
	set_position(_saved_rect.position());
	resize(_saved_rect.dimensions());
	_maximized = false;
	_window->set_draggable(true);
...
void Window::repaint_now() {
	...
	if(_decorated) {
		...
		// Draw X untuk close (pakai fill untuk garis diagonal)
		auto close_color = Theme::window_title();
		int margin = 4;
		// X diagonal kiri-atas ke kanan-bawah
		for(int i = 0; i < 2; i++) {
			for(int j = margin; j < button_size - margin; j++) {
				ctx.fill({_close_button.area.x + j + i, _close_button.area.y + j, 1, 1}, close_color);
			}
		}
		// X diagonal kanan-atas ke kiri-bawah
		for(int i = 0; i < 2; i++) {
			for(int j = margin; j < button_size - margin; j++) {
				ctx.fill({_close_button.area.x + (button_size - j - 1) + i, _close_button.area.y + j, 1, 1}, close_color);
			}
		}

		// Maximize button (tengah) - hanya tampil jika resizable dan show_min_max
		if(_resizable && _show_min_max_buttons) {
			_maximize_button.area = {
					_close_button.area.x - button_size - button_spacing,
					titlebar_rect.y + 3,
					button_size,
					button_size
			};
			ctx.draw_button_base(_maximize_button.area, _maximize_button.pressed, accent_color.darkened());
			
			// Draw maximize icon (kotak)
			int icon_margin = 4;
			if(_maximized) {
				// Restore icon: dua kotak overlap
				// Kotak belakang
				ctx.fill({_maximize_button.area.x + icon_margin + 2, _maximize_button.area.y + icon_margin, button_size - icon_margin * 2 - 2, 1}, close_color);
				ctx.fill({_maximize_button.area.x + icon_margin + 2, _maximize_button.area.y + icon_margin, 1, button_size - icon_margin * 2 - 2}, close_color);
				ctx.fill({_maximize_button.area.x + button_size - icon_margin, _maximize_button.area.y + icon_margin, 1, button_size - icon_margin * 2 - 2}, close_color);
				ctx.fill({_maximize_button.area.x + icon_margin + 2, _maximize_button.area.y + button_size - icon_margin - 2, button_size - icon_margin * 2 - 2, 1}, close_color);
				
				// Kotak depan
				ctx.fill({_maximize_button.area.x + icon_margin, _maximize_button.area.y + icon_margin + 2, button_size - icon_margin * 2 - 2, 1}, close_color);
				ctx.fill({_maximize_button.area.x + icon_margin, _maximize_button.area.y + icon_margin + 2, 1, button_size - icon_margin * 2 - 2}, close_color);
				ctx.fill({_maximize_button.area.x + button_size - icon_margin - 2, _maximize_button.area.y + icon_margin + 2, 1, button_size - icon_margin * 2 - 2}, close_color);
				ctx.fill({_maximize_button.area.x + icon_margin, _maximize_button.area.y + button_size - icon_margin, button_size - icon_margin * 2 - 2, 1}, close_color);
			} else {
				// Maximize icon: kotak tunggal
				ctx.fill({_maximize_button.area.x + icon_margin, _maximize_button.area.y + icon_margin, button_size - icon_margin * 2, 1}, close_color);
				ctx.fill({_maximize_button.area.x + icon_margin, _maximize_button.area.y + icon_margin, 1, button_size - icon_margin * 2}, close_color);
				ctx.fill({_maximize_button.area.x + button_size - icon_margin, _maximize_button.area.y + icon_margin, 1, button_size - icon_margin * 2}, close_color);
				ctx.fill({_maximize_button.area.x + icon_margin, _maximize_button.area.y + button_size - icon_margin, button_size - icon_margin * 2, 1}, close_color);
			}
		} else {
			_maximize_button.area = {0, 0, 0, 0};
		}
		...
		// Title bar text
		int button_total_width = button_size + button_spacing;
		if(_show_min_max_buttons && _resizable)
			button_total_width = (button_size + button_spacing) * 2;
		else
			button_total_width = button_size + button_spacing;
			
		auto title_rect = titlebar_rect.inset(4, button_total_width + 4, 4, title_xpos);
...
void Window::on_mouse_move(Pond::MouseMoveEvent event) {
	...
	if(_maximize_button.pressed && !_mouse.in(_maximize_button.area)) {
		_maximize_button.pressed = false;
		repaint();
	}

	bool draggable = !_mouse.in(_close_button.area) && 
	                 !_mouse.in(_maximize_button.area);
	...
	_window->set_draggable(draggable && !_maximized);
}
...
void Window::on_mouse_button(Pond::MouseButtonEvent evt) {
	...
	if(pressed_mouse1) {
		...
		if(_mouse.in(_close_button.area)) {
			...
			_window->set_draggable(false);
			repaint();
		} else if(_resizable && _mouse.in(_maximize_button.area)) {
			_maximize_button.pressed = true;
			_window->set_draggable(false);
			repaint();
		}
		...
		if(_close_button.pressed) {
			close();
			return;
		} else if(_maximize_button.pressed) {
			_maximize_button.pressed = false;
			if(_maximized)
				restore();
			else
