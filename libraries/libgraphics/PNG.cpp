...
void put_pixel(PNG* png, uint32_t color) {
  // BUG-04: guard against OOB write from corrupt/excess decompressor output
  if(png->pixel_x < 0 || png->pixel_x >= (int)png->ihdr.width ||
     png->pixel_y < 0 || png->pixel_y >= (int)png->ihdr.height)
    return;
...
void png_write(uint8_t byte, void* png_void) {
  ...
  if(png->state == STATE_SCANLINE_BEGIN) {
    ...
    // BUG-02: guard against pixel_buffer overflow (max 4 bytes)
    if(png->pixel_buffer_pos >= 4) return;
...
Framebuffer* Gfx::load_png_from_file(FILE* file) {
  ...
  while(1) {
    ...
    if(chunk == 0 && png.chunk_type != CHUNK_IHDR) {
      ...
      delete png.image; // BUG-03: free on all error paths
      return NULL;
    } else if(chunk == 0) {
      // Baca IHDR
...
Framebuffer* Gfx::load_png(const std::string& filename) {
  auto* file = fopen(filename.c_str(), "rb"); // BUG-01: binary mode required for PNG
  if(!file) {
