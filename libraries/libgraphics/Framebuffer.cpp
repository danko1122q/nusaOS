...
// FIX BUG #5: Ganti malloc dengan new[] agar konsisten dengan delete[]
void Framebuffer::deserialize(const uint8_t*& buf) {
	delete[] data;  // Sudah benar menggunakan delete[]
	auto* serialization = (FramebufferSerialization*) buf;
	width = serialization->width;
	height = serialization->height;
	if(width && height) {
		// FIX: Gunakan new[] bukan malloc, agar konsisten dengan delete[]
