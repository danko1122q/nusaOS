...
// Ukuran minimum DHCP packet yang valid:
// header (236 byte) + magic cookie (4 byte) + End option (1 byte) = 241
#define DHCP_HEADER_SIZE 236
#define DHCP_MIN_PACKET_SIZE (DHCP_HEADER_SIZE + 4 + 1)
...
void Client::loop() {
	...
	while (true) {
		...
		if (nread < (ssize_t) DHCP_MIN_PACKET_SIZE) {
			Duck::Log::errf("Received packet too small: {} bytes (minimum {})", nread, DHCP_MIN_PACKET_SIZE);
			continue;
		}
		...
		switch (type.value()) {
		case Offer:
			// FIX: handle Offer — balas dengan Request supaya server kirim Ack
			Duck::Log::infof("Received DHCP Offer from {}, sending Request...", buf.raw_packet().siaddr);
			res = do_offer(buf);
			break;
		...
		case Ack:
			res = do_ack(buf);
			if (!res.is_error()) {
				Duck::Log::success("DHCP configuration complete!");
				return;
			}
			break;

		case Nak:
			// Server tolak request kita, mulai ulang dari Discover
			Duck::Log::warnf("Received NAK from {}, restarting discovery...", buf.raw_packet().siaddr);
			for (auto& interface : m_interfaces)
				discover(interface);
			break;
...
Result Client::discover(const Interface& interface) {
	...
	Duck::Log::infof("Sending DHCP Discover on {}...", interface.name);
	return send_packet(interface, packet.raw_packet());
}

// FIX: implementasi do_offer — terima Offer, balas dengan Request
Result Client::do_offer(const DHCPPacket& packet) {
	// Cari transaksi yang sesuai dengan xid
	auto tx = m_transactions.find(packet.raw_packet().xid);
	if (tx == m_transactions.end())
		return {"Couldn't handle offer: No such transaction"};

	// Ambil IP yang ditawarkan server
	IPv4Address offered_ip = packet.raw_packet().yiaddr;

	// Ambil server identifier (IP server DHCP)
	auto server_id = packet.get_option<IPv4Address>(ServerID);
	if (!server_id.has_value())
		return {"Couldn't handle offer: No server ID in offer"};

	return request(tx->second.iface, tx->second.id, offered_ip, server_id.value());
}

// FIX: implementasi request — kirim DHCPREQUEST ke server
Result Client::request(const Interface& interface, int txid, const IPv4Address& offered_ip, const IPv4Address& server_ip) {
	DHCPPacket packet;
	packet.raw_packet().op = DHCPOp::BootPRequest;
	packet.raw_packet().htype = 1;
	packet.raw_packet().hlen = sizeof(MACAddress);
	packet.raw_packet().xid = txid; // pakai xid yang sama dengan Discover
	packet.raw_packet().flags = 0x1;
	// ciaddr harus 0 pada Request setelah Offer (belum punya IP resmi)
	packet.raw_packet().ciaddr = IPv4Address(0);
	for (int i = 0; i < 6; i++)
		packet.raw_packet().chaddr[i] = interface.hwaddr[i];

	uint8_t msgtype = DHCPMsgType::Request;
	packet.add_option(MessageType, 1, &msgtype);
	// Beritahu server IP mana yang kita minta
	packet.add_option(RequestedIP, sizeof(IPv4Address), &offered_ip);
	// Beritahu server mana yang kita pilih (penting jika ada beberapa server)
	packet.add_option(ServerID, sizeof(IPv4Address), &server_ip);
	packet.add_option(End, 0, nullptr);

	Duck::Log::infof("Sending DHCP Request for {} to server {}...", offered_ip, server_ip);
	return send_packet(interface, packet.raw_packet());
}
