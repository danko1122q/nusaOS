...
SoundServer::SoundServer() {
	...
	if(conn_res.is_error()) {
		Log::err("Couldn't connect to audio bus: ", conn_res.strerror()); // BUG FIX: pakai conn_res bukan bus_res
		exit(errno);
	}
...
void SoundServer::pump() {
	...
	// BUG FIX: pindahkan ke local static agar tidak polusi global scope
	static Sound::Sample mixed_samples[SOUNDCARD_BUFFER_SIZE];
	static uint32_t pcm_samples[SOUNDCARD_BUFFER_SIZE];
	...
	if (!m_soundcard.is_open())
		return; // BUG FIX: jangan tulis ke soundcard yang tidak terbuka
...
int SoundServer::request_buffer(sockid_t id) {
	// BUG FIX: gunakan find() untuk menghindari pembuatan entry null secara implisit
	auto it = m_clients.find(id);
	if(it == m_clients.end()) {
		Log::warn("request_buffer: client not found for sockid ", id);
		return -1;
	}
	return it->second->sample_buffer().buffer()->id();
}